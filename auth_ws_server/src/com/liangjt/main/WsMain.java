...
public class WsMain {
	...
	public static void main(String[] args) throws IOException {
		...
		// 添加in拦截器,记录日志的拦截器
		// ep.getInInterceptors().add(new LoggingInInterceptor(new
		// PrintWriter(new FileWriter("in.txt"))));
		// 添加out拦截器
		// ep.getOutInterceptors().add(new LoggingOutInterceptor(new
		// PrintWriter(new FileWriter("out.txt"))));

		// 添加in拦截器，该拦截器负责检查账号，密码
		ep.getInInterceptors().add(new com.liangjt.auth.AuthInInterceptor());
